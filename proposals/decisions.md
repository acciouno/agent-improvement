...
| 2026-09-14 | Fix passes C (content) + D (evaluator) from the second external review: 6 evaluator defects, log/placeholder/prose fixes | approved | Arjay explicitly chose "Apply the full fix list" | Done 2026-09-14: PR #1 proved the outer pin fires on a tampered eval script; PR #2 proved the mixed-change gate fires in isolation (pins+scripts green, only the gate failed). Both closed unmerged; probe branches deleted. |
